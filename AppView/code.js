...
$('#debug').on('click', function(){
  // readData(graphP.responseJSON[1]);
  // cy.layout(theLayout);
  cy.add([
    { group: "edges", data: { source: 'En applikation', target: 'Appen', type: 'spaghEdge' } },
  ]);
...
function readData2( data ){
  ...
  for(var i=0; i<data.edges.length; i++){
    ...
    incMidBarWidth();
    cy.add([
      { group: "nodes", data: { type: 'conPointNode', id:'sConP'+i}, position: {x: sourceConPosX, y: 100-i*15 }, selectable: false, locked: true },
      { group: "nodes", data: { type: 'conPointNode', id:'tConP'+i}, position: {x: targetConPosX, y: 100-i*15 }, selectable: false, locked: true },
